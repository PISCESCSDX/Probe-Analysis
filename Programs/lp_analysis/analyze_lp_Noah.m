...
%               resistance, ~line 196) by removing negative sign (negative
%               sign was used for etcher probe analysis.) 
%           -Adjusted default inputs for quicker use with CSDX plasmas. 
%           -Changed plotting limit in plotFitTE subfunction to add more
%               room on top of plots as they were getting cut off when
%               reading CSDX data. Instead of yMax = IA(end), 
%               yMax = max(IA) + 0.5;
%           - Changed beginning user prompt and final info display on
%               figure: main magnetic [A] ==> magnetic field [G] (Gauss).
%               Data used from CSDX is commonly saved with magnetic field
%               strength
%           - Added if/else statement to plotFitI_s subfunction to fix
...
%       Ar = (18,39.948)
%
% Commonly used default (for etcher)
%
% rfpower  =    '1300';       % [W]
% magnet   =      '50';       % [A]
% flow     =      '12';       % [sccm]
...
%
%
% Commonly used default (for CSDX with AR)
%
%rfpower  =    '1500';       % [W]
%magnet   =      '500';       % [G]
%flow     =      '25';       % [sccm]
...
%Res      =    '50.0';       % [Ohms]    Resistor current is measured across
%l_t      =  '2e-3';       %'2.19e-3';  % [m] length of tip
%d_t      =  '9e-4';       %'6.35e-4';  % [m] diameter of tip

%% ------------------------------------------------------
Element = 'Ar';

% default values for prompt (adjusted for CSDX)
rfpower  =    '1500';       % [W]
magnet   =      '500';       % [G]
flow     =      '25';       % [sccm]
pressure =     '3.2';       % [mT]
...
fid = fopen(file_vin);
V_p = textscan(fid,'%f','Delimiter', ' ');
fclose(fid);
V_p=cell2mat(V_p);

fid = fopen(file_vout);
I_p = textscan(fid,'%f','Delimiter', ' ');         % Still in Volts, must convert to current
fclose(fid);
I_p=cell2mat(I_p);
...
while choice == 1 
 % get T_e and E_sat fitting parameters from user       
  vals = getRange2(V_p,pFit.i3a,pFit.i3b,pFit.i4a,pFit.i4b);

%Use sliders to get the vals array along with the values for pFit
 pFit.v3a = vals(1);
 pFit.v3b = vals(2);
 pFit.i3a = iFind(V_p,pFit.v3a);
 pFit.i3b = iFind(V_p,pFit.v3b);
...
text(hrz1,hght-delh*3, 'magnetic field [G] = ','HorizontalAlignment','right');
...
function [Xvalue] = getRange2(VP,iMin,iMax,jMin,jMax)
Xvalue = zeros(4,1);
    minVi = VP(iMin);
    maxVi = VP(iMax);
    minVj = VP(jMin);
    maxVj = VP(jMax);
    Xvalue(1) = uicontrol('Parent',figure(1),'Style','slider','Position',[40,54,400,23],...
              'value',minVi, 'min',VP(1), 'max',VP(length(VP)));
    Xvalue(2) = uicontrol('Parent',figure(1),'Style','slider','Position',[480,54,400,23],...
              'value',maxVi, 'min',VP(1), 'max',VP(length(VP)));
    Xvalue(3) = uicontrol('Parent',figure(1),'Style','slider','Position',[920,54,400,23],...
              'value',minVj, 'min',VP(1), 'max',VP(length(VP)));
    Xvalue(4) = uicontrol('Parent',figure(1),'Style','slider','Position',[1360,54,400,23],...
              'value',maxVj, 'min',VP(1), 'max',VP(length(VP)));
...
xlim([xMin xMax])
%fixes axis problem that sometimes arises where yMin > yMax before fit is
%adjusted. Seems to happen when the Isat fit happens to start with a
%negative slope
if yMin < yMax
    ylim([yMin yMax])
else
