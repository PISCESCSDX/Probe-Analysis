function iudata = PlasmaFitIULangmuir_v33(data, checkonoff, ...
  fixpar, Vint, vol_avg, DIeexp_level)
%==========================================================================
%function iudata = PlasmaFitIULangmuir_v33(data, checkonoff, fixpar, ...
% Vint, vol_avg)
% Last Change: 2012-04-17 10:01 C. Brandt, San Diego
% - Working Version:
%   Advantage in this version: the fit parameters are only T_e and V_p.
%   ne is expressed by the ion density and included in the 
%   fit function. as a result ne is no fit parameter anymore.
%   (See details in the fit section below.)
%--------------------------------------------------------------------------
% PlasmaFitIULangmuir_v33 calculates the plasma parameters 
...
% vol_avg=6;
% iudata=PlasmaFitIULangmuir_v33(data,checkonoff,fixpar,Vint,vol_avg);
%==========================================================================
...
%--------------------------------------------------------------------------
[PolyIsat, ~, i_isat, Vf] = subPlasmaIsatPoly_v2(data);
...
%  i_fit1 = i_fit_ie_end;
  ...
  if Vint(1)==-Inf  % Left:  1
    i_fit0 = 1;
      i_w0 = Vf.i; %--------------- Start of Weights (in exponential region)
  end
  if isnan(Vint(2)) % Right: Vp
%    i_fit1 = i_fit_ie_end;
    i_fit1 = round((i_fit_ie_end + i_fit_ie_end_2)/2);
      i_w1 = i_fit1-Vf.i+i_w0; %End of Weights (in exponential region)
  end
  if Vint(2)==Inf   % Right: 1
    i_fit1 = numel(x);
      i_w1 = i_fit_ie_end-Vf.i+i_w0; %End of Weights (in exponential region)
  end
...
% fit formula: I_e(V) = I_e,sat * exp(-e (phi_p - V)/T_e)
%              I_e,sat = -e_0 * n_e * A_probe * <v+>
%              -->     = -e_0 * n_e * A_probe * sqrt(e_0 T_e/(2*pi*m_e))
%
% I_i,sat = 0.61 * e_0 * A_probe * n_i * sqrt(e_0 T_e/(m_i))
% n_i = I_i,sat / (0.61 *e_0 *A_probe * sqrt(e_0 T_e/(m_i)) )
% With n_e = n_i:
%   I_e(V) = -1/0.61 *I_i,sat *sqrt(m_i/(2*pi*m_e)) *exp(-e(phi_p-V)/T_e)
% a: n_e
% b: T_e
...
% 
% 16.4.2012 OLD: s1 = sprintf( '%5.5e', -e0*Ap*sqrt(e0/(2*pi*m_e)) );

% ni = Iisat_max/(0.61*e0*Ap*sqrt(e0/(data.m_ion*u0))) /sqrt(Te);
% ni = v2
v1 = -Iisat_max/0.61 * sqrt(data.m_ion*u0/(2*pi*m_e));
s1 = sprintf( '%5.5e', v1 );
fitfct = fittype([s1 '*exp(-(b - x)/a)']);
...
set(opts,'start',[Te_start  Vp_start]);
...
if isreal([T0 T1 Vp0 Vp1])
  if T1>=T0 && Vp1>=Vp0
    
    % Find the best fit depending on the fit range
    % Half-Length of Range variation:
    N2 = 5;
    ...
    for j=1:2*N2
      ...
      % 
      Vp0 = x(iloopfit(end)) + fixpar.Vplim(1);
      Vp1 = x(iloopfit(end)) + fixpar.Vplim(2);
      opts.Upper = [T1 Vp1];
      opts.Lower = [T0 Vp0];
      % Increase Weights of Fit in the exponential region:
      opts.weights = ones(1,numel(iloopfit));
      opts.weights(ind_w+j-N2) = 1000;
      % Fit whole electron current up to the plasma potential
      [~, gof] = fit(x(iloopfit), ye(iloopfit), fitfct, opts);
    ...
    % Find fit with the lowest deviation
    [~, iRmax] = max(Rsqr);
    ...
    % Increase Weights of Fit in the exponential region:
    opts.weights = ones(1,numel(iloopfit));
    opts.weights(ind_w+iRmax-N2) = 1000;
    % Fit whole electron current up to the plasma potential
    [ffit, gof] = fit(x(iloopfit), ye(iloopfit), fitfct, opts);
      % % Chi Square Deviation: gof.rsquare is exactly:
      % sumerrfit = sum( (yecut-Iefit(indcut)).^2 );
      % sumdiffmn = sum( (yecut - mean(yecut)).^2 );
      % iudata.Rsqr = 1 - sumerrfit/sumdiffmn;
    ...
    iudata.ne = Iisat_max/(0.61*e0*Ap*sqrt(e0*iudata.Te/(data.m_ion*u0)));
    iudata.Vf2= x(Vf.i);
    
    % Calculate the Ion Flux: Gamma_i = n_i*<v_th,i>
    iudata.Fli = iudata.ne*sqrt(2*e0*iudata.Te/(data.m_ion*u0));
        
    % Extract the confidence intervals
    c = confint(ffit);

    iudata.Teerr = [c(1,1) c(2,1)];
    iudata.Vperr = [c(1,2) c(2,2)];
