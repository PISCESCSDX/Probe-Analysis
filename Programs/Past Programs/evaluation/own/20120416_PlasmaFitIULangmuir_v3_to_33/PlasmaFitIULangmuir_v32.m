function [iudata, raw] = PlasmaFitIULangmuir_v32(data, checkonoff, ...
  fixpar, Vint, vol_avg, DIeexp_level)
%==========================================================================
%function [iudata] = PlasmaFitIULangmuir_v32(data, checkonoff, fixpar, ...
% Vint, vol_avg)
% Last Change: 2012-04-16 10:24 C. Brandt, San Diego
% - Test version: Insert variable length of fitting, end of fit is always
%   Vp
%--------------------------------------------------------------------------
% PlasmaFitIULangmuir_v3 calculates the plasma parameters 
...
% vol_avg=6;
% iudata=PlasmaFitIULangmuir_v3(data,checkonoff,fixpar,Vint,vol_avg);
%==========================================================================
...
if isreal([n0 n1 T0 T1 Vp0 Vp1])
  if n1>=n0 && T1>=T0 && Vp1>=Vp0
    ...
    for j=1:2*N2
     ...
     % 
     Vp1 = x(iloopfit(end)) + Vran;
     Vp0 = x(iloopfit(end)) - Vran;
    ...
    [~, iRmax] = max(Rsqr);
      ...
      ni = Iisat_max/(0.61*e0*Ap*sqrt(e0*Te(iRmax)/(data.m_ion*u0)));
      n1 =   2*ni;
      n0 = 0.5*ni;
      Vp1 = x(iloopfit(end))+Vran;
      Vp0 = x(iloopfit(end))-Vran;
      ...
      figure;
      subplot(2,1,1); plot(ye)
      subplot(2,1,2); plot(ind_fit(end)+(1:2*N2)-N2, Rsqr)
    ...
    % Calculate the ion density: I_i,sat=0.61*e0*ni*Ap*sqrt(e0*Te/mi)
    iudata.ni = Iisat_max/(0.61*e0*Ap*sqrt(e0*iudata.Te/(data.m_ion*u0)));
    % *** Add Error for ni
