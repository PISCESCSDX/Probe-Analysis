...
% # INPUT: Level-Range of Slope (parameter with strong influence on T_e!!)
  p_IexpLevel = 0.95;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
iudata = PlasmaFitIUDemidov_v11(data, chk, fixpar, Vint, vol_avg, ...
  p_IexpLevel);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%==========================================================================
...
[~, ir] = sort(rvec);
x = rvec(ir); y1 = R(ir); y2 = R2(ir);
ind = ~isnan(y1);
RR.x = x(ind); RR.y1 = y1(ind); RR.y2 = y2(ind);

% Save radial profiles
save([rawbase '_radprof.mat'],'rvec','ne','ni','Vf','Vf2','Vp','Te',...
  'Fli','RR')
...
%--------------------------
[~, ir] = sort(rvec);
x = rvec(ir); y = ne.n(ir); ey = (Te.e(:,2)-Te.e(:,1))./Te.n; ey = ey(ir);
ind = ~isnan(y);
x = x(ind); y = y(ind); ey = ey(ind);
%
ne.x = x; ne.y = y;
ne.fit.x = min(x):0.1:max(x);
ne.fit.y = csaps(x, y, 0.90, ne.fit.x, ey');
%--------------------------
% Ion density profile
%--------------------------
[~, ir] = sort(rvec);
...
%
ni.x = x; ni.y = y;
ni.fit.x = min(x):0.1:max(x);
ni.fit.y = csaps(x, y, 0.90, ni.fit.x);
%--------------------------
% Floating potential
%--------------------------
[~, ir] = sort(rvec);
...
Vf.x = x; Vf.y = y;
Vf.fit.x = min(x):0.1:max(x);
Vf.fit.y = csaps(x, y, 0.9, Vf.fit.x);
%------ Vf2 (from fit) -------
...
%
Vf2.x = x; Vf2.y = y;
Vf2.fit.x = min(x):0.1:max(x);
Vf2.fit.y = csaps(x, y, 0.9, Vf2.fit.x);
%--------------------------
% Plasma potential
%--------------------------
[~, ir] = sort(rvec);
x = rvec(ir); y = Vp.n(ir); ey = (Vp.e(:,2)-Vp.e(:,1)); ey = ey(ir);
ind = ~isnan(y);
x = x(ind); y = y(ind); ey = ey(ind);
%
%[x,y,~] = FctDeleteOutlier(x,y,0.15);
Vp.x = x; Vp.y = y;
Vp.fit.x = min(x):0.1:max(x);
Vp.fit.y = csaps(x, y, 0.9, Vp.fit.x, 1./ey');
%--------------------------
% Temperature profile
%--------------------------
x = rvec(ir); y = Te.n(ir); %ey = Te.e(ir); ***
  ey = (Te.e(:,2)-Te.e(:,1))./Te.n; ey = ey(ir);
ind = ~isnan(y);
x = x(ind); y = y(ind); ey = ey(ind);
%
Te.x = x; Te.y = y;
Te.fit.x = min(x):0.1:max(x);
Te.fit.y = csaps(x, y, 0.98, Te.fit.x, ey');
%--------------------------
% Ion Flux Profile
%--------------------------
...
%
Fli.x = x; Fli.y = y;
Fli.fit.x = min(x):0.1:max(x);
Fli.fit.y = csaps(x, y, 0.90, Fli.fit.x);
%--------------------------
% Regression coefficients
%--------------------------
% Averaged Regression coefficient
R = 0.5* (RR.y1/std(RR.y1) + RR.y2/std(RR.y2)) ;
%==========================================================================
...
mcol = [0 0 0];
log10max = floor( log10( max(ni.y) ) )-1;
hold on
  plot(ni.x, ni.y/(10^(log10max)), 'o', 'MarkerSize', MSz, ...
  'MarkerFaceColor', mcol, 'MarkerEdgeColor', 'w', 'LineWidth', 0.3)
  dx = [];
  dy = (10^(-log10max))* Rfac*abs([ni.y.*(1-R) ni.y.*(1-R)]);
  my_errorbar(ni.x,ni.y/(10^(log10max)),dx,dy,errwdth, mcol)
  plot(ni.fit.x, ni.fit.y/(10^log10max), 'k')
hold off
ymax = 1.1*max(ni.y)/(10^log10max); [dy, y1] = plotticks(0, ymax, 4);
ylim = [0 ymax]; ytick = y1:dy:ymax;
set(gca,'xlim',xlim,'xtick',xtick,'ylim',ylim,'ytick',ytick)
mkplotnice('r (cm)', ['n_i (10^{' num2str(log10max) '}m^{-3})'], ...
  fonts, '', '-26');

% --------- ne ---------
j = j+1; axes('position', ax{j})
mcol = [0 0 0];
log10max = floor( log10( max(ne.y) ) );
hold on
  plot(ne.x, ne.y/(10^log10max), 'o', 'MarkerSize', MSz, ...
  'MarkerFaceColor', mcol, 'MarkerEdgeColor', 'w', 'LineWidth', 0.3)
  dx = [];
  dy = abs(ne.e-[ne.n ne.n])/(10^log10max);
  my_errorbar(rvec, ne.n/(10^log10max), dx, dy, errwdth, mcol)
  plot(ne.fit.x, ne.fit.y/(10^log10max), 'k')
hold off
ymax = 1.1*max(ne.y)/(10^log10max); [dy, y1] = plotticks(0, ymax, 4);
ylim = [0 ymax]; ytick = y1:dy:ymax;
set(gca,'xlim',xlim,'xtick',xtick,'ylim',ylim,'ytick',ytick)
mkplotnice('r (cm)', ['n_e (10^{' num2str(log10max) '}m^{-3})'], ...
  fonts, '', '-26');
...
j = j+1; axes('position', ax{j})
mcol = [0 0 0];
log10max = floor( log10( max(Fli.y) ) );
hold on
  plot(rvec, Fli.n/(10^(log10max)), 'o', 'MarkerSize', MSz, ...
  'MarkerFaceColor', mcol, 'MarkerEdgeColor', 'w', 'LineWidth', 0.3)
  dx = [];
  dy = (10^(-log10max))* Rfac*abs([Fli.y.*(1-R) Fli.y.*(1-R)]);
  my_errorbar(Fli.x,Fli.y/(10^(log10max)),dx,dy,errwdth, mcol)
  plot(Fli.fit.x, Fli.fit.y/(10^(log10max)), 'k')
hold off
ymin = 0; ymax = 1.1*max(Fli.y/(10^log10max));
[dy, y1] = plotticks(ymin, ymax, 4);
ylim = [ymin ymax]; ytick = y1:dy:ymax;
set(gca,'xlim',xlim,'xtick',xtick,'ylim',ylim,'ytick',ytick)
mkplotnice('r (cm)', ['\Gamma_i (10^{' num2str(log10max) ...
  '}m^{-2}s^{-1})'], fonts, '', '-26');
...
plot(Vf.fit.x, Vf.fit.y, 'Color', 'c')
hold off
...
plot(rvec, Vp.n, 'o', 'MarkerSize', MSz, ...
  'MarkerFaceColor', mcol, 'MarkerEdgeColor', 'w', 'LineWidth', 0.3)
  dy = abs(Vp.e-[Vp.n Vp.n]);
  my_errorbar(rvec, Vp.n, dx, dy, errwdth, mcol)
plot(Vp.fit.x, Vp.fit.y, 'Color', mcol)
hold off
set(gca,'xlim',xlim,'xtick',xtick,'ylim',ylim,'ytick',ytick)
