...
hold on
plot(Dyelog, 'b-o')
plot(ind_Ie, Dyelog_cut_sm, 'r-o')
plot(ind_Ieexp_max-i_Ieexp_start+1, ...
  Dyelog_cut_sm(ind_Ieexp_max-i_Ieexp_start+1), 'r*')
hold off
set(gca,'xlim',[1 numel(yelog)])
mkplotnice('', '', 12, '0', '0');
...
hold on;
plot(D2yelog, 'b-o')
plot(ind_Ie, D2yelog_sm, 'r-o')
plot(i_Iesat_start, D2yelog(i_Iesat_start), 'r*')
hold off
set(gca,'xlim',[1 numel(yelog)])
mkplotnice('', '', 12, '0', '0');
...
figeps(10,8,2);clf;
subplot(2,1,1)
plot( real(log(-y+polyval(PolyIsat,x))) , 'b-*')
set(gca,'xlim',[1 numel(yelog)])
mkplotnice('', '', 12, '0', '0');
...
line(ind_Ieexp_max*[1 1] , 1.1*[min(yelog(ind)) max(yelog(ind))], 'Color', 'b')
...
%    opts.Lower = [n0 T0];
    ...
    iVp = iVp+5;
    opts.weights(iVf:iVp) = 1e2;
    [ffit, gof] = fit(x(1:iVp), ye(1:iVp), fitfct, opts);
...
% ****     % Calculate fit function along original x-vector (here e-current)
     ...
     figure(4); clf;
     hold on
     plot(x(1:iVp), ye(1:iVp), 'co-')
     plot(x(iVp:end), ye(iVp:end), 'bo-')
     plot(x(1:iVp), ffit(x(1:iVp)), 'r')
     hold off
     
     raw.Te4 = ffit.b;
     disp(['Te: min, avg, fit: ' num2str([raw.Te2 raw.Te3 raw.Te4])])
