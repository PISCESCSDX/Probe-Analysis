function [iudata, raw] = PlasmaFitIULangmuir_v2(data, checkonoff, ...
...
%    - Start of exponential electron region (end of noise)
...
%     i_Ieexp_start_1: rough estimate
%     i_Ieexp_start_2: start exp. Ie (more right than i..1)
...
% (everything larger belongs to noise fluct. in the ion saturation regime)
  ...
  ind = AbsD2yelog<avg;
  AbsD2yelog(ind) = 0;
  % Help procedure: Smooth and Filter strongly: gets zero at Ie
  % (if a d2-maximum is at the end, it will be ignored by this procedure)
    func = filtspline(x, AbsD2yelog, round(5/dx), 1);
    func(1: round(0.1*numel(func)) ) = 1;
    func0= func<eps;
    fund0= diff_discrete(x,func0);
    fpeak= fund0>eps;
    [~,imax,~,~] = extrema(AbsD2yelog);
    helpi =   imax<max( indx(fpeak) );
% i_Ieexp_start_1: Start of growth above the noise level:
%(corresponds in some degree to start of expon. growth of electron current)
i_Ieexp_start_1 = max( imax(helpi) );

if isempty(i_Ieexp_start_1)
 disp('Did not find the start of the exponential growth! No Ev!')
  iudata = []; raw = [];
  return
end

% figeps(15,12,2,40,0); clf;
% hold on
% plot(AbsD2yelog,'b-')
% plot(i_Ieexp_start_1, AbsD2yelog(i_Ieexp_start_1),'ro')
% hold off

% Average and Maximum of the noisy ion saturation region
  yelog_noise_avg = mean(yelog(1:i_Ieexp_start_1));
  yelog_noise_max = max(yelog(1:i_Ieexp_start_1));
...
i_Ieexp_start_2 = inum(1);
% New cut electron current regime
ind_Ie = i_Ieexp_start_2:length(yelog);
...
%     (
  ii = imax >= i_Ieexp_start_1-2*pts; ii = imax(ii);
...
subplot(2,1,1)
hold on
  plot(1:i_fit_ie_end, 1e3*ye(1:i_fit_ie_end), 'co-')
  plot(1:i_fit_ie_end, 1e3*ffit(x(1:i_fit_ie_end)), 'r')
  %
  plot(i_fit_ie_end+1:numel(x), 1e3*ye(i_fit_ie_end+1:end), 'bo-')
hold off
