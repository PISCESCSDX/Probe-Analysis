...
% Vint, vol_avg)
% Last Change: 2012-04-11 18:49 C. Brandt, San Diego
% - Version works somehow reliable, but fit range could be better!
% - ne is not in the order of magnitude with Daisuke's calculation
%--------------------------------------------------------------------------
...
%     i_Ieexp_start_2: start exp. Ie (more to right hand side than i_Iee*1)
...
% (everything larger belongs to noise fluct. in the ion saturation regime)
  ...
  i_Ieexp_start_1 = pkpos(ind);
  % --- Check Plots: Detection of the Noise level End Positon ----
  %   close all; figure(1); clf;
  %   hold on
  %   plot(AbsD2yelog);
  %   plot(i_Ieexp_start_1,AbsD2yelog(i_Ieexp_start_1), 'r*')
  %   hold off
  %   input('Press')
  % --
