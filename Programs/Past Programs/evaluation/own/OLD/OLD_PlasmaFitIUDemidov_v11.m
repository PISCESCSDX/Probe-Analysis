function iudata = PlasmaFitIUDemidov_v11(data, p_chk, p_fix, p_Vint, ...
  p_Vavg, p_IexpLevel, p_IsRg)
%==========================================================================
%function iudata = PlasmaFitIUDemidov_d11(data, p_chk, p_fix, p_Vint, ...
% p_Vavg, p_IexpLevel)
% Last Change: 2012-05-02 11:11 C. Brandt, San Diego
% This is a alpha version: v11.
% In this version the determination of the 
...
%--------------------------------------------------------------------------
% Ion saturation current range (in Volt)
...
if isreal([cn0 cn1 T0 T1])
  if cn1>=cn0 && T1>=T0
    ...
    %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    % Find Optimal Fit of the fit loop with Best Plasma Potential:
    %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    [ymax, iRmax] = max(RR);
     ...
     disp('No fit: No best fit is found.')
            iudata.ni  = NaN;
    ...
    % Define plasma potential (at end of region) and fit function
    vp = x(iloopfit(end));
    ffun = @(c,x) c(1)*(vp-x)./c(2).*exp(-(vp-x)./c(2));
    ub = [cn1 T1];  lb = [cn0 T0];
    % FINAL FIT: Electron current up to the plasma potential
      ...
      % Calculate the energy distribution function
      
      iudata.EDF.x = xx;
      iudata.EDF.y = -3/(8*sqrt(2)*pi) *(m_e^(3/2)/(e0^(5/2))) * ...
            r_p*log(pi*L_p/(4*r_p)) *data.B ./ ((vp-xx).^(3/2)) .* yy;
          
      iudata.EDFfit.x = xx;
      iudata.EDFfit.y = -3/(8*sqrt(2)*pi) *(m_e^(3/2)/(e0^(5/2))) * ...
            r_p*log(pi*L_p/(4*r_p)) *data.B ./ ((vp-xx).^(3/2)) .* Dyefit;

      iudata.EDF.raw.x = x;
      iudata.EDF.raw.y  = -3/(8*sqrt(2)*pi) *(m_e^(3/2)/(e0^(5/2))) * ...
            r_p*log(pi*L_p/(4*r_p)) *data.B .* Dye;
          
    % Druyvesteyn method: Calculate f(E) by 2nd derivative
    DDye = diff_discrete(x, Dye);
    DDye = filtmooth(DDye, 10);
    DDye = DDye(iloopfit);
    iudata.EDF.Dru.y = 4*m_i/(A_p*(e0^2)) .* (vp-xx) .* DDye;
    iudata.EDF.Dru.x = xx;
    %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
