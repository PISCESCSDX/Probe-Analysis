function iudata = PlasmaFitIULangmuir_a10(data, p_chk, ...
  p_fix, p_Vint, p_Vavg, p_IexpLevel, p_ni)
%==========================================================================
%function iudata = PlasmaFitIULangmuir_a10(data, p_chk, p_fix, ...
% p_Vint, p_Vavg)
% Last Change: 2012-04-20 09:48 C. Brandt, San Diego
% This is a working Version: a10. (last working version before: v33)
% This version provides the option 'p_ni' of determining 
% (See details in the fit section below.)
%--------------------------------------------------------------------------
% PlasmaFitIULangmuir_a10 calculates the plasma parameters 
...
% p_Vavg=1;
% iudata=PlasmaFitIULangmuir_a10(data,p_chk,p_fix,p_Vint,p_Vavg);
%==========================================================================
...
if strcmp(p_ni,'Ie')
  fitfct = fittype('a*sqrt(b)*exp(-(c-x)/b)');
end
%-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
%     f(x) = v1*exp(-(a-x)/b)
if strcmp(p_ni,'Iisat')
  v1 = -Iisat_max/0.61*sqrt(m_i/(2*pi*m_e));
  s1 = sprintf( '%5.5e', v1 );
  fitfct = fittype([s1 '*exp(-(b-x)/a)']);
end

opts = fitoptions(fitfct);
% set(opts, 'Algorithm', 'Levenberg-Marquardt')
%  scaleProblem to 'none' to choose Equation 6-126, and 
%  set ScaleProblem to 'Jacobian'
  %set(opts, 'Robust', 'On')
%   set(opts, 'Robust', 'Bisquare')
%--------------------------------------------------------------------------
...
% Start parameter n_e at Plasma potential:
%   n_e = -I_e(V_p)/[A_p*e0*sqrt{e0*Te_start/(2*pi*m_e)}]
if isempty(p_fix.ne)
...
if isempty(p_fix.Vplim)
  % Vp will be not varied
  Vp0 = Vp_start;
  Vp1 = Vp_start;
else
  Vp0 = Vp_start + p_fix.Vplim(1);
  Vp1 = Vp_start + p_fix.Vplim(2);
end
...
if isreal([n0 n1 T0 T1 Vp0 Vp1])
  if n0>=n1 && T1>=T0 && Vp1>=Vp0
    ...
    for j=1:2*N2
      ...
      % 
      Vp0 = x(iloopfit(end)) + p_fix.Vplim(1);
      Vp1 = x(iloopfit(end)) + p_fix.Vplim(2);
      % Switch between fit procedure (A) and (B)
      switch p_ni
        case 'Ie'
...
%         opts.weights = 1*ones(1,numel(iloopfit));
...
%         opts.Upper = [n1 T1 Vp1];
%         opts.Lower = [n0 T0 Vp0];
%         opts.Upper = [4.6e16 16 -10];
%         opts.Lower = [4.4e16 14 -11];
      case 'Iisat'
        opts.Upper = [T1 Vp1];
        opts.Lower = [T0 Vp0];
    end
...
%     opts.weights(ind_w+iRmax-N2) = 1000;
    ...
    switch p_ni
      case 'Ie'
        iudata.ne = ffit.a/(-e0*A_p*sqrt(e0/(2*pi*m_e)));
        iudata.Te = ffit.b;
        iudata.Vp = ffit.c;
