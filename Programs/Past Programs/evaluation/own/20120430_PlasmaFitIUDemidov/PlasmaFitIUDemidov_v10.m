function iudata = PlasmaFitIUDemidov_v10(data, p_chk, p_fix, p_Vint, ...
  p_Vavg, p_IexpLevel)
%==========================================================================
%function iudata = PlasmaFitIULangmuir_a11(data, p_chk, p_fix, p_Vint, ...
% p_Vavg, p_IexpLevel)
% Last Change: 2012-04-24 17:05 C. Brandt, San Diego
% This is a beta Version: v10. (last working version before: none)
% In this version another fit routine is used ('lsqcurvefit') since it
% seems to work more reliable than the normal 'fit' method.
% (See details in the fit section below.)
%--------------------------------------------------------------------------
% PlasmaFitIUDemidov_v10 calculates the plasma parameters
...
% p_Vavg=1;
% iudata=PlasmaFitIUDemidov_v10(data,p_chk,p_fix,p_Vint,p_Vavg);
%==========================================================================
...
if isreal([cn0 cn1 T0 T1])
  if cn1>=cn0 && T1>=T0
    ...
    Rsqr2 = Rsqr1; Rstd2 = Rstd1; Rrat2 = Rrat1;
    %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    ...
    for j=1:2*N2
      ...
      xx = x(iloopfit); yy = Dyesm(iloopfit);
    ...
    iudata.Vperr = iudata.Vp*[1 1]; % *** wdth Rrat1-peak below 95% max
    iudata.Vf= Vf.V;
