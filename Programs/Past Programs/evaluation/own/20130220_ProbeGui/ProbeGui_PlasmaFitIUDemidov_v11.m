function iudata = ProbeGui_PlasmaFitIUDemidov_v11(data, p_chk, p_fix, ...
  p_Vint, p_Vavg, p_IsRg, p_fitgoal, p_scanstepsize, handles)
%==========================================================================
% function iudata = ProbeGui_PlasmaFitIUDemidov_v11(data, p_chk, p_fix, ...
%   p_Vint, p_Vavg, p_IsRg, p_fitgoal, p_stepsize, handles)
%
% Last Change: 2013-02-20 16:35 C. Brandt, San Diego
% This is alpha version: v11. It is based on
% the .m-file PlasmaFitIUDemidov_v11
%--------------------------------------------------------------------------
...
if isreal([cn0 cn1 T0 T1])
  if cn1>=cn0 && T1>=T0
    ...
    Rsqr2 = Rsqr1; Rstd2 = Rstd1; Vploop = Rsqr1;    
          ...
          axes(handles.axes1); clf
          hold on
          plot(handles.axes1,xfit,yefit,'bo');
          hp = plot(0,0,'r');
          hold off
...
clf; subplot(2,1,1)
hold on
  plot(xind    ,1e3*ye   ,'o-','Color', rgb('NavajoWhite'))
  plot(xifitres,1e3*Iefit,'r-','LineWidth', 2)
hold off
d = max(ye)-min(ye);
set(gca, 'ylim', 1e3*[min(ye)-0.02*d max(ye)+0.02*d])
set(gca, 'xlim', [1 ll])
...
hold on
  ...
  yf = 1e3*ffun(ffit{iRmax},x(xifitres));
